        ...
        if self.rank == 0 and global_step > 0 and global_step % self.frequent == 0:
            if self.init:
                ...
                if self.writer is not None:
                    ...
                    self.writer.add_scalar('easy_num', one, global_step)
                    self.writer.add_scalar('noise_num', two, global_step)
                    self.writer.add_scalar('hard_num', three, global_step)
                    self.writer.add_scalar('m_', four, global_step)
                if fp16:
                    msg = "Speed %.2f samples/sec   Loss %.4f   LearningRate %.6f   Epoch: %d   Global Step: %d   " \
                          "Fp16 Grad Scale: %2.f   Required: %1.f hours     easy_num %.2f   noise_num %.2f    hard_num %.2f  m_ %4f" % (
                              speed_total, loss, learning_rate, epoch, global_step,
                              grad_scaler.get_scale(), time_for_end, one, two, three,four
                          )
                else:
                    msg = "Speed %.2f samples/sec   Loss %.4f   LearningRate %.6f   Epoch: %d   Global Step: %d   " \
                          "Required: %1.f hours" % (
